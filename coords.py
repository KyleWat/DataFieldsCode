...
class GeographicSystem(CoordinateSystem):
    ...
    def toECEF(self, lon, lat, alt):
        ...
        transformer = proj4.Transformer.from_proj(self.ERSlla, self.ERSxyz)
        projectedData = array(transformer.transform(lon, lat, alt ))
    ...
    def fromECEF(self, x, y, z):
        ...
        transformer = proj4.Transformer.from_proj(self.ERSxyz, self.ERSlla)
        projectedData = array(transformer.transform(x, y, z ))
        if len(projectedData.shape) == 1:
            return projectedData[0], projectedData[1], projectedData[2]
        else:
            return projectedData[0,:], projectedData[1,:], projectedData[2,:]
...
class MapProjection(CoordinateSystem):
    ...
    def toECEF(self, x, y, z):
        ...
        y += self.cy
        z += self.cz
        transformer = proj4.Transformer.from_proj(self.projection, self.ERSxyz)
        projectedData = array(transformer.transform(x, y, z ))
        if len(projectedData.shape) == 1:
    ...
    def fromECEF(self, x, y, z):
        transformer = proj4.Transformer.from_proj(self.ERSxyz, self.projection)
        projectedData = array(transformer.transform(x, y, z ))
        if len(projectedData.shape) == 1:
            px, py, pz = projectedData[0], projectedData[1], projectedData[2]
        else:
...
class GeostationaryFixedGridSystem(CoordinateSystem):
    ...
    def toECEF(self, x, y, z):
        ...
        transformer = proj4.Transformer.from_proj(self.fixedgrid, self.ECEFxyz)
        return transformer.transform(X, Y, Z)

    def fromECEF(self, x, y, z):
        transformer = proj4.Transformer.from_proj(self.ECEFxyz, self.fixedgrid)
...
class RadarCoordinateSystem(CoordinateSystem):
    ...
    def __init__(self, ctrLat, ctrLon, ctrAlt, datum='WGS84', ellps='WGS84', effectiveRadiusMultiplier=4./3.):
        ...
        transformer = proj4.Transformer.from_proj(self.lla,self.xyz)
        self.Requator, foo1, foo2 = transformer.transform(0,0,0) # Equatorial radius  - WGS-84 value = 6378137.0
        foo1, foo2, self.Rpolar = transformer.transform(0,90,0) # Polar radius  - WGS-84 value = 6356752.314
...
class TangentPlaneCartesianSystem(object):
    ...
    def __init__(self, ctrLat=0.0, ctrLon=0.0, ctrAlt=0.0):
        ...
        transformer = proj4.Transformer.from_proj(ERSlla, ERSxyz)
        
        self.centerECEF = array(transformer.transform(ctrLon, ctrLat, ctrAlt))

        #location of point directly above local center
        aboveCenterECEF = array(transformer.transform(ctrLon, ctrLat, self.ctrAlt+1e3))
        ...
        northCenterECEF = array(transformer.transform(self.ctrLon, self.ctrLat+1.01, self.ctrAlt))
